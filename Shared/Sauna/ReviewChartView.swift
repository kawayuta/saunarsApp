    ...
    func updateUIView(_ uiView: RadarChartView, context: Context) {
        ...
        if let sauna = viewModel.sauna {
            ...
            if cleanlinessAverage != 0 {
                cleanlinessAverage /= sauna.reviews.count
                customerServiceAverage /= sauna.reviews.count
                equipmentAverage /= sauna.reviews.count
                customerMannerAverage /= sauna.reviews.count
                costPerformanceAverage /= sauna.reviews.count
